...
@Component({
  ...
  standalone: true,
  imports: [IonSpinner, IonContent, IonTitle, IonIcon, CommonModule, FormsModule, RideSelectionComponent]
})
...
@ViewChild('dropInput') dropInput!: ElementRef<HTMLInputElement>;
  ...
  private checkMapSelectedLocation() {
    const stateLoc = history.state?.selectedLocation || history.state?.data?.selectedLocation;
    let locationData = stateLoc;

    if (!locationData) {
      try {
        const stored = localStorage.getItem('ride_selected_location');
        if (stored) {
    ...
    if (locationData && (locationData.lat || locationData.address)) {
      const target: 'Pickup' | 'Drop' = locationData.target || this.selectedInput;
      const coords = {
        lat: Number(locationData.lat),
        lng: Number(locationData.lng)
      };
      const name = locationData.name || locationData.area || 'Selected Location';
      const address = locationData.address || name;
      ...
      if (target === 'Pickup') {
        ...
        this.tripData.origin = this.pickupLocation as any;
        if (!this.drop) {
        ...
        this.tripData.drop = this.dropLocation as any;
        if (!this.pickup) {
          this.selectedInput = 'Pickup';
        }
      }

      localStorage.removeItem('ride_selected_location');
      localStorage.removeItem('map_picker_source');
      localStorage.removeItem('ride_picker_target');

      // If both pickup and drop are ready, transition to ride selection
      if (this.pickup && this.drop && this.pickupCoords && this.dropCoords) {
  ...
  openSelectOnMap() {
    const currentCoords = this.selectedInput === 'Pickup' ? this.pickupCoords : this.dropCoords;
    localStorage.setItem('map_picker_source', 'ride');
    localStorage.setItem('ride_picker_target', this.selectedInput);
    this.navCtrl.navigateForward('/layout/map', {
      state: {
        data: {
          source: 'ride',
          target: this.selectedInput,
  ...
  swapLocations() {
    ...
    this.onInputChange(this.selectedInput);
  }

  clearInput(type: 'Pickup' | 'Drop') {
    if (type === 'Pickup') {
      this.pickup = '';
      this.pickupCoords = undefined;
      this.pickupLocation = null;
      this.tripData.origin = '' as any;
    } else {
      this.drop = '';
      this.dropCoords = undefined;
      this.dropLocation = null;
      this.tripData.drop = '' as any;
    }
  ...
  async onInputChange(type: 'Pickup' | 'Drop') {
    ...
    // Filter User History immediately
    ...
    this.debounceTimer = setTimeout(() => {
      ...
      const biasCoords = type === 'Drop' ? (this.pickupCoords || this.dropCoords) : this.pickupCoords;
      if (biasCoords) {
          ...
          this.ngZone.run(async () => {
            ...
            try {
              const places = await Promise.all(
                predictions.slice(0, 6).map(p => this.getPlaceDetails(p))
              );
    ...
    return new Promise(resolve => {
      ...
      this.placesService.getDetails({ placeId: prediction.place_id, fields: ['name', 'formatted_address', 'geometry'] }, (place, status) => {
        if (status === google.maps.places.PlacesServiceStatus.OK && place?.geometry?.location) {
          ...
          const isInside = this.polygon && (window as any).google?.maps?.geometry?.poly ?
            google.maps.geometry.poly.containsLocation(loc, this.polygon) : true;
          resolve({
            name: place.name || prediction.structured_formatting?.main_text,
            formatted_address: place.formatted_address || prediction.structured_formatting?.secondary_text,
            geometry: {
              location: {
                lat: loc.lat(),
                lng: loc.lng()
